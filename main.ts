...
type Employee = {                                                        //created a type Employee with its properties
    name  : string ,
    department : string ,
    role : Role,
    contact? : {                                                        //optional nested object with its properties
        phone : number,
        email : string,
    }
    skills? : string[],
...
type Role = "Manager"|"Engineer"|"Intern"|"Student";                       //created a union type Role with restrict properties

let employee : Employee = {                                              //an emplpoyee object with its data
    name: "Rameen",
...
type Car = {                                                    //creating a car type which represent car object
    name : string ,
    model : string ,
    year : number ,
    color : string ,
    engine : {                                                     // engine is nested car object
        horsePower : number ,
    }
    getHorsePower : () => any ;                                    // a function in  car object
}
 
let myCar : Car = {                                            //putting values in  car object
    name: "Honda Civic",
    model : "Standard",
    year : 2024 ,
    color : "Black",
...
type Product = {                                                      //creating a type Product which represent T-shirt Object
    name : string,
    price : number,
    color : string
    inventory : {                                                   //nested object inventory
        stock : number,                                             //Represents the number of T-shirts available.
        colorOptions? : string[],                                   //optional array of strings for colorOptions
        changeColor : (newColor:string) => void                     //change color function declaration
    };
}

let shirt : Product = {                                              //values inserting in t-shirt object 
    name : "POLO T-Shirt",
    price : 2000,
    color :"black",
    inventory : {
        ...
        changeColor : (newColor:string) => {                            //function implementation
